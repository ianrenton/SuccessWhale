...
// Grab session objects if they exist (i.e. generated by entry point 2 / 3, or
// this is not the user's first refresh of index.php this session.
...
if (DB_ENABLED) {
    if (!empty($_SESSION['thisUser'])) { // Catch a weird error case, keep the DB clean if we see it
        mysql_connect(DB_SERVER,DB_USER,DB_PASS);
        @mysql_select_db(DB_NAME) or die( "Unable to select database");
        ...
        if (mysql_num_rows($result) > 0) {
            if (isset($_SESSION['twitter_access_token'])) {
                $query = "UPDATE users SET auth_token = '" . mysql_real_escape_string(serialize($_SESSION['twitter_access_token'])) . "' WHERE username = '" . mysql_real_escape_string($_SESSION['thisUser']) . "'";
                mysql_query($query);
            }
            if (isset($facebook)) {
