...
$content .= '<div style="width: 60%; margin:50px auto 0px auto;"><p>SuccessWhale allows users to cache their Twitter / Facebook etc. authentication tokens, and protect them with a password.  This is useful for users that wish to use SuccessWhale in places where <code>twitter.com</code> is blocked.  This form allows users that are having their authentication tokens cached to log in and retrieve that token, bypassing the need to visit twitter.com to authenticate.</p><p>If you would like to use this feature, you must first log in using the normal method from a PC where <code>twitter.com</code> is available.  Once logged in, click the "Cache Auth Tokens" link at the top-right, and set a password.  You will then be able to log in using this form from any PC.</p></div>';
...
if (isset($_GET['fail'])) {
    $content .= '<p align="center" style="margin-top:50px; color:red;"><strong>Sorry!  Either that user has not opted to cache their tokens, or the password you entered was incorrect.</strong></p>';
}
